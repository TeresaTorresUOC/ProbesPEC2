...
export class CategoryService {
  ...
  getCategoriesByUserId(userId: string | number): Observable<CategoryDTO[]> {
    return this.http.get<CategoryDTO[]>(`${this.urlBlogUocApi}?userId=${userId}`);
  }
  ...
  updateCategory(categoryId: string, category: CategoryDTO): Observable<CategoryDTO> {
    return this.http.put<CategoryDTO>(`${this.urlBlogUocApi}/${encodeURIComponent(String(categoryId))}`, category);
  }
