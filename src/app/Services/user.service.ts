...
export class UserService {
  ...
  getAllUsers(): Observable<UserDTO[]> {
    return this.http.get<UserDTO[]>(this.urlBlogUocApi);
  }

  getUsersByEmail(email: string): Observable<UserDTO[]> {
    return this.http.get<UserDTO[]>(`${this.urlBlogUocApi}?email=${email}`);
  }

  getUsersByName(name: string): Observable<UserDTO[]> {
    return this.http.get<UserDTO[]>(`${this.urlBlogUocApi}?name=${name}`);
  }
}
